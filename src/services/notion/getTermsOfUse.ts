...
export async function getTermsOfUse(): Promise<GetTermsOfUseResponse> {
  ...
  return await notionClient('getTermsOfUse', async () => {
    const filters = [{
      property: "Slug",
      rich_text: {
        equals: 'terms-of-use',
    ...
    return {
      ...
      date: props.Date.date?.start ? date(props.Date.date.start).format(dateFormat) : "",
      timeFromDate: props.Date.date?.start ? date(props.Date.date.start).locale(dateLocale).fromNow() : "",
