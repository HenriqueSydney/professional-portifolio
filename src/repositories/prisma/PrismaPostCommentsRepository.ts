...
import {
  FetchCommentsByPostIdResponse,
  IPostCommentsReposity,
  PostCommentesWithUser,
} from "../IPostCommentsRepository";

export class PrismaPostCommentsRepository implements IPostCommentsReposity {
  async create(
    data: Prisma.PostCommentsUncheckedCreateInput
  ...
  async deleteById(id: number): Promise<{ postId: number } | null> {
    ...
    if (!postCommentData) return null;
    ...
    return { postId: postCommentData.postId };
  }
  ...
  async incrementLikeToCommentByCommentId(
    id: number,
    userId: string
  ): Promise<void> {
    await prisma.$transaction([
  ...
  async decrementLikeToCommentByCommentId(
    id: number,
    userId: string
  ): Promise<void> {
    await prisma.$transaction([
  ...
  async incrementLikeOfANotLoggedUserToCommentByCommentId(
    id: number
  ): Promise<void> {
    await prisma.$transaction([
      prisma.postComments.update({
  ...
  async decrementLikeOfANotLoggedUserToCommentByCommentId(
    id: number
  ): Promise<void> {
    await prisma.$transaction([
      prisma.postComments.update({
