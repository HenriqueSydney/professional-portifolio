...
export class RedisClient implements IRedisClient {
  ...
  private initMetrics(): void {
    ...
    this.connectionGauge = this.meter.createUpDownCounter(
    ...
    this.cacheHitRatio = this.meter.createCounter(
  ...
  generateCacheKey(prefix: string, tags: string[], params?: any): string {
    ...
    const orderedTags = [...tags].sort((a, b) => a.localeCompare(b));
    const tagString = tags.length > 0 ? `:${orderedTags.join(":")}` : "";
      ...
      async () => {
        try {
          ...
          if (totalOfKeys > 0) {
            await pipeline.exec();
            this.logger.info(
              `Cache invalidado para tags: ${tags.join(", ")}, ${totalOfKeys} chaves removidas`
            );
          } else {
            this.logger.info(
              `Nenhuma chave encontrada para as tags: ${tags.join(", ")}`
            );
          }
        } catch (error) {
          this.logger.error(
      ...
      async () => {
        try {
          ...
          if (keys.length > 0) {
            ...
            this.logger.info(
              `${keys.length} chaves removidas para o prefixo: ${prefix}`
            );
          } else {
            this.logger.info(
              `Nenhuma chave encontrada para o prefixo: ${prefix}`
            );
          }
        } catch (error) {
          this.logger.error(
  ...
  async isConnected(): Promise<boolean> {
    return await this.executeRedisOperation("ping", async () => {
      try {
  ...
  async disconnect(): Promise<void> {
    return await this.executeRedisOperation("quit", async () => {
      try {
