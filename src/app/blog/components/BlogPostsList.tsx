...
import { BlogPost, fetchBlogPosts } from "@/services/notion/fetchBlogPosts"
...
export function BlogPostsList({ posts, hasMorePosts, postsNextCursor }: IBlogPosts) {
  ...
  const selectedCategory = params.get('category') as "Todos" | "DevOps" | "Monitoring" | "Architecture" | "Security" | "Infrastructure" | "Frontend" || 'Todos'
  const query = params.get('query')
  const favorites = params.get('favorites') === 'true'
  const fixed = params.get('fixed') === 'true'

  // Reset posts quando os filtros mudarem
  useEffect(() => {
    setAllPosts(posts)
    setHasMore(hasMorePosts)
    setNextCursor(postsNextCursor)
    setError(null)
  }, [selectedCategory, query, favorites, fixed, posts, hasMorePosts, postsNextCursor])
  ...
  }, [isLoading, hasMore, nextCursor, selectedCategory, query, favorites, fixed])
  ...
  return (
    <div>
      ...
      <div ref={sentinelRef} className="h-10 flex items-center justify-center mt-20">
        {isLoading && (
