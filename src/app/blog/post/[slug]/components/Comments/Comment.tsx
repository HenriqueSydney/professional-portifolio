...
import { CheckCircle, Edit2, Frown, ThumbsUp, Trash, XCircle } from "lucide-react"
...
export function Comment({ comment }: IComment) {
    ...
    const [lastEditedTimeOfComment, setLastEditedTimeOfComment] = useState<Date | null>(null)
    ...
    function handleOptimistcCommentText(editedComment: string, operationDateTime: Date | null) {
    ...
    async function handleRemoveComment() {
        ...
        toast({
            variant: "destructive",
            title: "Erro ao tentar remover o comentário",
            action: <XCircle className="h-7 w-7 text-destructive-foreground" />
        })
    ...
    const canEdit = date().diff(date(comment.updatedAt), "minute") < 30 && sessionData?.user.id === comment.userId
    return (

        <AnimatedCollapseDiv>
            <div

                className="rounded-xl border  bg-card p-4 shadow-sm hover:bg-zinc-800/60 transition"
            >
                        ...
                        <div>
                            ...
                            {date(comment.createdAt).isSame(comment.updatedAt, "second") && (
                                <p className="text-sm text-muted-foreground">
                                    Postado {date(comment.createdAt).fromNow()}
                                </p>
                            )}
                            {date(comment.createdAt).isBefore(comment.updatedAt, "second") && (
                                <p className="text-sm text-muted-foreground">
                                    Post editado {date(lastEditedTimeOfComment).fromNow() ?? date(comment.updatedAt).fromNow()}
                                </p>
                            )}
                        </div>
                    </div>
                ...
                {!isEditFormOpen && (
                    <AnimatedCollapseDiv>
                        <p className={`mt-3 text-sm leading-relaxed text-card-foreground ${expandedComments.includes(comment.id) ? "" : "line-clamp-3"}`}>
