...
export function TableOfContents() {
  ...
  const [lastAnchorVisible, setLastAnchorVisible] = useState<string | null>(null)


  const anchorIds = headersInfo.map(h => `#${h.id}`)
  const anchors = useVisibleAnchor(anchorIds, { threshold: 0.3, rootMargin: '-100px 0px -40% 0px' })
